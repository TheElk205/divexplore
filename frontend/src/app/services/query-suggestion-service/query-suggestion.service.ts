...
//const URL = GlobalConstants.querySuggestionServerURL;
...
export class QuerySuggestionConnectionService {
  ...
  constructor(private globalConstants: GlobalConstantsService) {
    console.log('QuerySuggestionConnectionService created');
    this.messages = this.connectToServer();
  }

  public connectToServer() {
    this.messages = <Subject<Message>>this.connectToWebsocket(this.globalConstants.querySuggestionServerURL).pipe(
      map(
        (response: MessageEvent): Message => {
          //console.log(`Query Suggestion  server: ${response.data}`);
  ...
  private create(url: string): AnonymousSubject<MessageEvent> {
    ...
    let observable = new Observable((obs: Observer<MessageEvent>) => {
      ws.onopen = (e) => {
        ...
        console.log("Connected to Query Suggestion Server: " + url);
        let msg = { 'data': JSON.stringify(statusConnected) };
        obs.next(new MessageEvent('message', msg));
      }
      ws.onmessage = (msg) => {
        console.log('message from Query Suggestion  server');
        obs.next(msg);
      };
      ws.onerror = (e) => {
        console.log('Error with Query Suggestion  server');
        obs.error(obs);
      };
      ws.onclose = (e) => {
        console.log('Disconnected from Query Suggestion  server');
    ...
    let observer: Observer<MessageEvent> = {
      ...
      complete: () => { },
      next: (data: Object) => {
        //console.log('Sent to Query Suggestion  server: ', data);
